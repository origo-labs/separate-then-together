...
class CollaborationStrategy(ABC):
    ...
    @abstractmethod
    def filter_history(
        self, full_history: List[Dict[str, Any]], current_agent_name: str
    ) -> List[Dict[str, Any]]:
...
class SeparateStrategy(CollaborationStrategy):
    ...
    def filter_history(
        ...
        """
        return [entry for entry in full_history if entry.get("role") == current_agent_name]
...
class SeparateTogetherStrategy(CollaborationStrategy):
    ...
    def __init__(self, separate_turns: int = 6, collab_turns: int = 11):
